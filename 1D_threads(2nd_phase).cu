...
//
__global__ void mutrixMulKernel(const int *m_d, const int *n_d, int * p_d, int width,int block_dim_x)
{
	int pvalue = 0;
	int y = threadIdx.x % block_dim_x;
	int x = threadIdx.x / block_dim_x;
	int split_size_x = width / block_dim_x;
	int split_size_y = width / block_dim_x;
	...
	for (int i = x_offset; i < (x_offset + split_size_x); i++)
	{
		...
		for (int j = y_offset; j < y_offset + split_size_y; j++)
		{
			...
			{
				int melement = m_d[i * width + k];
				int nelement = n_d[k * width + j];
				pvalue += melement * nelement;
			}

			p_d[i * width + j] = pvalue;
		}
	}
}
...
cudaError_t multiplyGPU(DataSet dataSet, int num_threads)
{
	...
	if ((sqrtNTH*sqrtNTH != num_threads) || (N % sqrtNTH != 0)||(sqrtNTH>N))
	{
		fprintf(stderr, "INAPPROPRIATE NUMBER OF THREADS  / MATRICE SIZE !\n");
		goto Error;
	}

	int blockdim_x = sqrtNTH;

	mutrixMulKernel <<< 1, num_threads >>>(m_d, n_d, p_d, N,blockdim_x);
